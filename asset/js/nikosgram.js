$(function () {
    ...
    $.getJSON(_SCOPE[1] + 'streams/' + _USERNAME[1], function (live) {
        if (live['stream'] != null) {
            ...
            $('[data-header]').css('background-image', 'url("' + stream['preview']['template'].replace('{width}x{height}', '1920x1080') + '")');
            ...
            $.getJSON(live['_links']['channel'], function (channel) {
                $('[data-header]').css('background-image', 'url("' + channel['video_banner'].replace('640x360', '1920x1080') + '")');
